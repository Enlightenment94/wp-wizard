...
function report($filePath){
    ...
    $response = "";
    $response .= "response_code: " . $responseCode . "</br>";
    if (isset($report->positives) && $report->positives > 0) {
        $response .= $filePath . " : " . "malicious"  . " : " .$report->positives . "</br>\n";
    } elseif($responseCode == 0) {
        $response .= $filePath . " : " . "queue" . "</br>\n";
    } elseif($responseCode == -2) {
        $response .= $filePath . " : " . "analyst" . "</br>\n";
    }else{
        $response .= $filePath . " : " . "clean" . "</br>\n";
    }
    $guiLink = "https://www.virustotal.com/gui/file/" . $fileHash . "/detection";

    $response .= "<a href='" . $guiLink . "'>" . $guiLink . "</a>";
...
function scanVt($filePath){
    ...
    if($responseCode[0] == 0){
        ...
        return array($responseCode[1], $body);
    }else{
        $body = "";
        return array($responseCode[1], $body);;
    }
